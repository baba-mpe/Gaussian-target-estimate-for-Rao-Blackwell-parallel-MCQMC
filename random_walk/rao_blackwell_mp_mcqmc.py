...
def rb_mp_mcqmc(x0, N, stepSize, PowerOfTwo, Stream,  mu_target, Sigma_target, WeightIn=0):
    ...
    for n in range(numOfIter):
        ...
        y = xI + norm.ppf(U[:,:d], loc=np.zeros(d), scale=stepSize)
        ...
        LogK_ni = -0.5/(stepSize**2)*np.dot(np.dot(proposals-xI, np.identity(d)), \
                             (proposals-xI).T).diagonal(0)
        ...
        if N>1:
            ...
            I = np.searchsorted(PstatesSum, U[:N,d:].flatten())[0]
            xI = proposals[I]
